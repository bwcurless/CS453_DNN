// To run this program, pass in the file path to the dataset and the neural network will train on the dataset, and then the accuracy will be evaluated.
...
int main(int argc, char *argv[])
{
	...
	// The network is essentially constructed from memory allocated to store all the data as it propagates through the different layers, and the kernels that implement the different layers, affine, ReLu, softmax, convolutional

	// Allocate memory for all intermediate steps on the GPU. This includes caching inputs to each layer, outputs, and gradients used for backpropagation
	// Input layer
	...
	for (int epoch = 0; epoch < NUMEPOCHS; epoch++) {
		...
		for (int batch = 0; batch < ceil(1.0 * TRAINSIZE / MINIBATCHSIZE) ; batch++) {
			...
			//gpuErrchk(cudaMemcpy(dev_inputLayer, &trainData[minibatchStartIndex], sizeof(float) * MINIBATCHSIZE);
			
	
		
	
	// Perform gradient descent here
	// TODO Optional perform Stochastic gradient descent and iterate on a minibatch of inputs to speed up convergence

	// Take all images in training dataset and pass them through the network
	// Each layer will cache it's gradient in the pre allocated memory space as we go to prepare for backpropogation
	

	// At this point we will have the loss computed for every input image, and the gradient of our softmax function. We now begin to backpropogate the gradients
	// Backpropogate the gradient with respect to all parameters all the way through the network
	...
	// Evaluate accuracy of classifier on test dataset (Don't really need to do this since this is mostly just for fun)
