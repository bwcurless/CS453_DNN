...
nvcc -O3 -arch=compute_80 -code=sm_80 -lcuda -Xcompiler -fopenmp -lineinfo -Xcompiler -rdynamic affineLayerTests.cu ../../src/affineLayer.cu -link -o "$outputFile"
...
nvcc -O3 -arch=compute_80 -code=sm_80 -lcuda -lineinfo -Xcompiler -fopenmp -Xcompiler -rdynamic affineLayerTests.cu ../../src/affineLayer.cu -link -o "$outputFile"
