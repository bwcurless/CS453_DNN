...
nvcc -O3 -arch=compute_80 -code=sm_80 -lcuda -Xcompiler -fopenmp -lineinfo -Xcompiler -rdynamic ReLULayerTests.cu ../../src/ReLULayer.cu -link -o "$outputFile"

# Define where outputs go
outputPath="/scratch/emb538/"
...
nvcc -O3 -arch=compute_80 -code=sm_80 -lcuda -lineinfo -Xcompiler -fopenmp -Xcompiler -rdynamic ReLULayerTests.cu ../../src/ReLULayer.cu -link -o "$outputFile"

srun "./$outputFile"
#compute-sanitizer --tool=memcheck "./$outputFile"
# -f overwrite profile if it exists
#srun ncu -f -o "ReLULayer_Profile" --clock-control=none --set full "./$outputFile"
